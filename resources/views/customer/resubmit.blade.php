{{-- resources/views/customer/booknow.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
        ...
        <div class="sidebar-content">
            <nav class="nav flex-column mt-4">
                ...
                <a class="nav-link active" href="#">
                    <i class="fas fa-clipboard-list"></i> <span>Book Vehicle</span>
                </a>
    ...
    <div class="main-content">
            ...
            <div class="user-info d-flex align-items-center">
                <div class="dropdown">
                    <a href="#" class="d-flex align-items-center text-decoration-none dropdown-toggle" data-bs-toggle="dropdown">
                        ...
                        <span class="ms-2 d-none d-sm-inline fw-semibold">{{ session('user_name') ?? 'Customer' }}</span>
                    </a>
        ...
        <div class="content-area">
            ...
            <!-- Resubmit Alert -->
            @if(isset($is_resubmit) && $is_resubmit && isset($resubmit_booking))
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                <i class="fas fa-undo-alt me-2"></i> 
                <strong>Resubmitting Booking #{{ $resubmit_booking->id }}</strong><br>
                Your previous booking was rejected. You can modify the details and resubmit.
                @if($resubmit_booking->rejection_reason)
                <br><strong>Rejection Reason:</strong> {{ $resubmit_booking->rejection_reason }}
                @endif
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            </div>
            <input type="hidden" name="resubmit_booking_id" id="resubmit_booking_id" value="{{ $resubmit_booking->id }}">
            @endif

            @if(isset($vehicle) || request()->has('vehicle_id'))
            <!-- Vehicle Summary -->
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="vehicle-summary-image" style="background-image: url('{{ isset($vehicle) && $vehicle->vehicle_image ? asset('storage/'.$vehicle->vehicle_image) : 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=500' }}')">
                                <div class="vehicle-summary-badge">
                                    @if(isset($vehicle) && $vehicle->is_booked == 'no')
                                        Available
                                    @else
                                        Available
                                    @endif
                        ...
                        <div class="col-md-8">
                            <div class="vehicle-summary-info">
                                <h5>{{ isset($vehicle) ? $vehicle->vehicle_type : 'Truck' }} - {{ isset($vehicle) ? $vehicle->vehicle_number : 'N/A' }}</h5>
                                <div class="row mt-3">
                                    <div class="col-4">
                                        <small class="text-muted">Driver</small>
                                        <p class="mb-0">{{ isset($vehicle) && $vehicle->user ? $vehicle->user->name : 'N/A' }}</p>
                                    </div>
                                    <div class="col-4">
                                        <small class="text-muted">Capacity</small>
                                        <p class="mb-0">{{ isset($vehicle) ? ($vehicle->weight_capacity ?? 'N/A') : 'N/A' }} kg</p>
                                    </div>
                                    <div class="col-4">
                                        <small class="text-muted">Rate/km</small>
                                        <p class="mb-0">Rs {{ isset($vehicle) ? ($vehicle->rate_per_km ?? '35') : '35' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('trip.submit') }}" method="POST" id="bookingForm">
                        ...
                        <!-- Hidden fields -->
                        <input type="hidden" id="pickup_lat" name="pickup_lat" value="{{ isset($resubmit_booking) ? $resubmit_booking->pickup_lat : '' }}">
                        <input type="hidden" id="pickup_lng" name="pickup_lng" value="{{ isset($resubmit_booking) ? $resubmit_booking->pickup_lng : '' }}">
                        <input type="hidden" id="destination_lat" name="destination_lat" value="{{ isset($resubmit_booking) ? $resubmit_booking->dropoff_lat : '' }}">
                        <input type="hidden" id="destination_lng" name="destination_lng" value="{{ isset($resubmit_booking) ? $resubmit_booking->dropoff_lng : '' }}">
                        <input type="hidden" id="estimated_distance" name="estimated_distance" value="{{ isset($resubmit_booking) ? $resubmit_booking->estimated_distance : '' }}">
                        <input type="hidden" id="estimated_fare" name="estimated_fare" value="{{ isset($resubmit_booking) ? $resubmit_booking->estimated_fare : '' }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" id="trip_date" name="trip_date" 
                                       min="{{ date('Y-m-d') }}" 
                                       value="{{ isset($resubmit_booking) ? $resubmit_booking->booking_date : date('Y-m-d', strtotime('+1 day')) }}" 
                                       required>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="time" class="form-control" id="pickup_time" name="pickup_time" 
                                       value="{{ isset($resubmit_booking) ? $resubmit_booking->pickup_time : '09:00' }}">
                            </div>
                            ...
                            <div class="col-md-6 position-relative">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="pickup_location" name="pickup_location" 
                                           ...
                                           value="{{ isset($resubmit_booking) ? $resubmit_booking->pickup_location : '' }}" 
                            ...
                            <div class="col-md-6 position-relative">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="drop_location" name="drop_location" 
                                           ...
                                           value="{{ isset($resubmit_booking) ? $resubmit_booking->dropoff_location : '' }}" 
                                           required autocomplete="off">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="goods_type" name="goods_type">
                                    ...
                                    <option value="furniture" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'furniture' ? 'selected' : '' }}>Furniture</option>
                                    <option value="electronics" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'electronics' ? 'selected' : '' }}>Electronics</option>
                                    <option value="textiles" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'textiles' ? 'selected' : '' }}>Textiles</option>
                                    <option value="machinery" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'machinery' ? 'selected' : '' }}>Machinery</option>
                                    <option value="food" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'food' ? 'selected' : '' }}>Food Items</option>
                                    <option value="construction" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'construction' ? 'selected' : '' }}>Construction Materials</option>
                                    <option value="other" {{ isset($resubmit_booking) && $resubmit_booking->goods_type == 'other' ? 'selected' : '' }}>Other</option>
                                </select>
                            </div>
                            
                            <div class="col-md-6">
                                <label class="form-label">Goods Weight (kg)</label>
                                <input type="number" step="0.01" class="form-control" id="goods_weight" name="goods_weight" 
                                       placeholder="Enter weight"
                                       value="{{ isset($resubmit_booking) ? $resubmit_booking->goods_weight : '' }}">
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="special_instructions" name="special_instructions" 
                                          rows="2" placeholder="Any special instructions...">{{ isset($resubmit_booking) ? $resubmit_booking->special_instructions : '' }}</textarea>
                            </div>
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-4 text-center text-md-start mb-3 mb-md-0">
                                            <div class="distance-badge">
                                                ...
                                                <span id="distance_display">{{ isset($resubmit_booking) && $resubmit_booking->estimated_distance ? $resubmit_booking->estimated_distance . ' km' : '-- km' }}</span>
                                            </div>
                                            <div class="mt-2">
                                        ...
                                        <div class="col-md-8 text-center text-md-end">
                                            ...
                                            <div class="fare-amount" id="fare_display">{{ isset($resubmit_booking) && $resubmit_booking->estimated_fare ? 'Rs ' . number_format($resubmit_booking->estimated_fare) : '--' }}</div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="payment_method" name="payment_method" required>
                                    ...
                                    <option value="jazzcash" {{ isset($resubmit_booking) && $resubmit_booking->payment_method == 'jazzcash' ? 'selected' : '' }}>JazzCash</option>
                                    <option value="easypaisa" {{ isset($resubmit_booking) && $resubmit_booking->payment_method == 'easypaisa' ? 'selected' : '' }}>Easypaisa</option>
                                    <option value="cod" {{ isset($resubmit_booking) && $resubmit_booking->payment_method == 'cod' ? 'selected' : '' }}>Cash on Delivery</option>
                                </select>
                            </div>
                            ...
                            <div class="col-12 text-end mt-4">
                                ...
                                <button type="submit" class="btn btn-success" id="submitBtn" 
                                        {{ isset($resubmit_booking) && $resubmit_booking->estimated_distance ? '' : 'disabled' }}>
                                    <i class="fas fa-check-circle me-1"></i> 
                                    {{ isset($is_resubmit) && $is_resubmit ? 'Resubmit Booking' : 'Submit Booking' }}
                                </button>
                            </div>
                        </div>
                    </form>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check for resubmit data from rejected booking
            @if(isset($resubmit_booking) && $resubmit_booking)
                @if($resubmit_booking->pickup_lat && $resubmit_booking->pickup_lng)
                setTimeout(function() {
                    const pickupLat = parseFloat('{{ $resubmit_booking->pickup_lat }}');
                    const pickupLng = parseFloat('{{ $resubmit_booking->pickup_lng }}');
                    const destLat = parseFloat('{{ $resubmit_booking->dropoff_lat }}');
                    const destLng = parseFloat('{{ $resubmit_booking->dropoff_lng }}');
                    
                    if (pickupLat && pickupLng) {
                        updateMarker('pickup', pickupLat, pickupLng, '{{ addslashes($resubmit_booking->pickup_location) }}');
                    }
                    if (destLat && destLng) {
                        updateMarker('destination', destLat, destLng, '{{ addslashes($resubmit_booking->dropoff_location) }}');
                    }
        ...
        function formatInstruction(instruction) {
            ...
            let text = instruction.replace(/<[^>]*>/g, '');
            ...
            const routes = [
                {
                    ...
                    summary: 'Via Motorway - Fastest route with tolls',
                    directions: [
                ...
                },
                {
                    ...
                    has_tolls: false,
                    toll_cost: 0,
                    summary: 'Via local roads - Avoid tolls, longer but economical',
                    directions: [
                        { instruction: 'Start from pickup location', distance: 0, duration: 0 },
                        { instruction: 'Head towards main road', distance: 2000, duration: 120 },
                        { instruction: 'Continue on local roads', distance: 60000, duration: 3600 },
        ...
        window.selectRoute = function(routeId) {
            ...
            // Update fare and distance display
        ...
        function setDefaultDateTime() {
            ...
            if (!document.getElementById('trip_date').value) {
